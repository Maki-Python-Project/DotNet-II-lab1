    ...
    internal class Database
    {
        public static async Task Main(string[] args)
        {
           ...
           await sqlConnection.OpenAsync();
            ...
            command.CommandText = "SELECT Readers.Surname, Readers.Name, Readers.PatronymicName, Readers.Address, Readers.Phone, Readers.DateBirth, Tickets.ID, Books.Author, Books.Name as book, InfoBooks.DateTakeBook, InfoBooks.DateReturnBook, Books.PriceFROM Books INNER JOIN InfoBooks ON Books.ID = InfoBooks.BookID INNER JOIN Tickets ON InfoBooks.TicketID = Tickets.ID INNER JOIN Readers ON Tickets.ReaderID = Readers.ID ORDER BY Readers.Surname, Readers.Name, Readers.PatronymicName";
